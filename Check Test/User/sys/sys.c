...
void sys_init(void)
{
    ...
    if (app_config[0] != 0X01 ||
        app_config[1] != 0xFE) //前两位不是标志位，代表混乱或者没有做过初始化
    {
        SPI_FLASH_BulkErase(); //擦除FLASH
        delay_ms(1000);
        SPI_FLASH_BufferWrite(Default_config, SYS_INFO_ADDR, 20); //写入默认设置
        SPI_FLASH_BufferRead(app_config, SYS_INFO_ADDR,
                             20); //将默认设置读取到app_config数组
        ...
        app_config[10] ==
        0x01) //前两位是标志位，但是10位上仍然是0x01即未初始化情况，进入INIT
    {
        FEEDBACK[2] = 0x00; //第二位为0x00是需要初始化的反馈
        FeedBack(FEEDBACK, USART1, 5); //发送需要信息的指令
        SYS_STATUS = Sys_INIT;
